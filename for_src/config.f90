...
subroutine set_initial_conditions
 ...
 ! set eddy spectrum, just mode 1 in vertical
 L_loc = N0*Lz/pi/f0
...
!                                          b3(is_pe:ie_pe,js_pe:je_pe,ks_pe:ke_pe)
 ...
 cspeed = sqrt(om2)/sqrt(kx(i)**2+ky(j)**2)
 if (my_pe==0) then
  ...
  print*,'  wave speed = ',cspeed,' m/s'
  print*,' time to cross basin = ',Lx/cspeed/86400.,' days'
